...
def tokenize_single(link):
    tokenizer = Tokenizer(lower=True, char_level=True, oov_token='-n-')
    tokenizer.fit_on_texts(link)
    sequence_length = 200
    tokenized_link = pad_sequences(tokenizer.texts_to_sequences(link), maxlen=sequence_length)
    print("tokenized link: ", tokenized_link)
    print("length of tokenized link is", len(tokenized_link))
